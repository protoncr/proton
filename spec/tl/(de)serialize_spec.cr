  ...
  describe Array do
    describe "#tl_serialize" do
      it "serializes vector to tl value" do
        expect(Array(Int32).new.to_bytes).to eq(Bytes[0x15, 0xc4, 0xb5, 0x1c, 0x0, 0x0, 0x0, 0x0])
        expect([0x7f_i32].to_bytes).to eq(Bytes[0x15, 0xc4, 0xb5, 0x1c, 0x1, 0x0, 0x0, 0x0, 0x7f, 0x0, 0x0, 0x0])
      end
  ...
  describe Array do
    describe ".tl_deserialize" do
      it "deserializes vector from tl value" do
        expect(Array(Int32).from_bytes(Bytes[0x15, 0xc4, 0xb5, 0x1c, 0x0, 0x0, 0x0, 0x0])).to eq(Array(Int32).new)
        expect(Array(Int32).from_bytes(Bytes[0x15, 0xc4, 0xb5, 0x1c, 0x1, 0x0, 0x0, 0x0, 0x7f, 0x0, 0x0, 0x0])).to eq([0x7f_i32])
      end
