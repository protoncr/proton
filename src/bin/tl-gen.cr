require "json"
require "./tl_gen/generator"

GEN_TL      = Bool.from_json(ENV.fetch("PROTON_GEN_TL", "true"))
TL_API_FILE = ENV.fetch("PROTON_TL_API_FILE", Path[__DIR__, "./tl_gen/data/api.tl"].expand.to_s)

GEN_MT      = Bool.from_json(ENV.fetch("PROTON_GEN_MT", "true"))
MT_API_FILE = ENV.fetch("PROTON_MT_API_FILE", Path[__DIR__, "./tl_gen/data/mtproto.tl"].expand.to_s)
