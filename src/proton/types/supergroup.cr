...
module Proton::Types
  ...
  # username [::String, nil] Username of the supergroup or channel; empty for private supergroups or channels.
  # date [::Int32] Point in time (Unix timestamp) when the current user joined, or the point in time when the
  #   supergroup or channel was created, in case the user is not a member.
  # status [Proton::Types::ChatMemberStatus] Status of the current user in the supergroup or channel.
  # member_count [::Int32] Member count; 0 if unknown.
  #   Currently it is guaranteed to be known only if the supergroup or channel was found through SearchPublicChats.
  # sign_messages [::Bool] True, if messages sent to the channel should contain information about the sender.
  ...
  # restriction_reason [::String] If non-empty, contains the reason why access to this supergroup or channel must be
  #   restricted.
