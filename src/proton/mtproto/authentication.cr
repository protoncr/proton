module Proton
  module MTProto
    module Authentication
      ...
      # :nodoc:
      def self.do_step1(random_bytes)
        nonce = BigInt.from_bytes(random_bytes, 128)
        {TL::Root::ReqPqMulti.new(nonce).to_bytes, Step1.new(random_bytes)}
      end
      ...
      def self.do_step2(data, response, random_bytes)
        ...
        pq_inner_data = TL::Root::PQInnerData.new(
          ...
          new_nonce: BigInt.from_bytes(new_nonce, 256)).to_bytes
        ...
        if !fingerprint
          raise UnknownFingerprintsError.new(fingerprints: res_pq.server_public_key_fingerprints)
        end
        ...
        req_dh_params = TL::Root::ReqDHParams.new(
          nonce: BigInt.from_bytes(nonce, 128),
          server_nonce: res_pq.server_nonce,
          p: p_bytes,
          q: q_bytes,
      ...
      def self.do_step3(data : Step2, response : Bytes, random_bytes : Bytes, now : Int32)
        ...
        server_dh_params = TL::Root::ReqDHParams.return_type.from_bytes(response)
        ...
        begin
          server_dh_inner = TL::Root::ServerDHInnerData.tl_deserialize(plain_text_io)
        rescue ex : Error
          raise InvalidDhInnerDataError.new(ex)
        end
        ...
        if got_answer_hash != expected_answer_hash
          raise InvalidAnswerHashError.new(got: got_answer_hash, expected: expected_answer_hash)
        end
        ...
        set_client_dh_params = TL::Root::SetClientDHParams.new(
          BigInt.from_bytes(nonce, 128),
          BigInt.from_bytes(server_nonce, 128),
      ...
      def self.create_key(data : Step3, response : Bytes)
        ...
        dh_gen = TL::Root::SetClientDHParams.return_type.from_bytes(response)
        dh_gen = case dh_gen
        ...
        if dh_gen.nonce_number == 1
          ...
          raise DHGenFailError.new
        end
      end

      def self.check_nonce(got : Bytes, expected : Bytes)
        if got != expected
          raise InvalidNonceError.new(got, expected)
        end
        true
      end
      ...
      def self.check_server_nonce(got : Bytes, expected : Bytes)
        if got != expected
          raise InvalidServerNonceError.new(got, expected)
        end
        true
      end
      ...
      def self.check_new_nonce_hash(got : Bytes, expected : Bytes)
        if got != expected
          raise InvalidNewNonceHashError.new(got, expected)
        end
        true
      end
      ...
      def self.check_g_in_range(value, low, high)
        if low >= value || value >= high
          raise GParameterOutOfRangeError.new(value, low, high)
        end
        true
      end
