module Proton::MTProto
  # An implementation of the [Mobile Transport Protocol](https://core.telegram.org)
  # for ciphertext (encrypted) messages.
  class Encrypted
    include Mtp
    ...
    CONTAINER_HEADER_LEN = (8 + 4 + 4) + (4 + 4)
    ...
    property auth_key : Crypto::AuthKey
    ...
    property last_msg_id : Int64

    # Identifiers that need to be acknowledged to the server.
    #
    # A [Content-related Message](https://core.telegram.org/mtproto/description#content-related-message)
    # is "a message requiring an explicit acknowledgment. These include all the user
    # and many service messages, virtually all with the exception of
    # containers and acknowledgments."
    property pending_ack : Array(Int64)
    ...
    property rpc_results : Array(Tuple(Int64, Bytes | Error::RequestError))

    # Temporary updates that came in a response.
    property updates : Array(Bytes)

    # Buffer were requests are pushed to.
    property buffer : IO::Memory

    # How many messages are there in the buffer.
    property msg_count : UInt32

    def initialize(@auth_key, @time_offset, @salt, @client_id, @sequence, @last_msg_id, @pending_ack,
                   @compression_threshold, @rpc_results, @updates, @buffer, @msg_count)
    end
    ...
    def get_new_msg_id
      now = Time.utc - Time::UNIX_EPOCH

      seconds = (now.total_seconds.to_i + self.time_offset).to_u64
      nanoseconds = now.nanoseconds.to_u64
      new_msg_id = ((seconds << 32) | (nanoseconds << 2)).to_i64
    ...
    def serialize_msg(body : Bytes, content_related : Bool)
      msg_id = self.get_new_msg_id

      msg_id.tl_serialize(self.buffer)
      self.get_seq_no(content_related).tl_serialize(self.buffer)
      body.size.tl_serialize(self.buffer)
    ...
    def finalize_plain
      return Bytes.empty if self.msg_count.zero?

      buffer = self.buffer.to_slice
      puts buffer

      if self.msg_count == 1
        # Won't be writing the message for the container.
        buffer = buffer[CONTAINER_HEADER_LEN...]
      end

      begin
        tmp = IO::Memory.new(HEADER_LEN)
        self.salt.tl_serialize(tmp)      # 8 bytes
        self.client_id.tl_serialize(tmp) # 8 bytes
        buffer[0...HEADER_LEN].copy_from(tmp.to_slice)
      end

      if self.msg_count != 1
        # Give the container its message ID and sequence number.
        tmp = IO::Memory.new(CONTAINER_HEADER_LEN)

        # Manually `serialize_msg` because the container body was already written.
        self.get_new_msg_id.tl_serialize(tmp)
        self.get_seq_no(false).tl_serialize(tmp)

        # + 8 because it has to include the constructor ID and length (4 bytes each).
        len = (self.buffer.size - HEADER_LEN - CONTAINER_HEADER_LEN + 8)
        len.tl_serialize(tmp)

        MessageContainer::CONSTRUCTOR_ID.tl_serialize(tmp)
        self.msg_count.tl_serialize(tmp)
        buffer[HEADER_LEN ... (HEADER_LEN + CONTAINER_HEADER_LEN)].copy_from(tmp.to_slice)
      end
    ...
    def process_message(message : Message)
      ...
      when TL::Root::BadMsgNotification::CONSTRUCTOR_ID, TL::Root::BadServerSalt::CONSTRUCTOR_ID
      ...
      when TL::Root::MsgDetailedInfo::CONSTRUCTOR_ID, TL::Root::MsgNewDetailedInfo::CONSTRUCTOR_ID
        self.handle_detailed_info(message)
        # Explicit Request to Re-Send Messages & Explicit Request to Re-Send Answers
      when TL::Root::MsgResendReq::CONSTRUCTOR_ID, TL::Root::MsgResendAnsReq::CONSTRUCTOR_ID
      ...
      when TL::Root::DestroySessionOk::CONSTRUCTOR_ID, TL::Root::DestroySessionNone::CONSTRUCTOR_ID
    ...
    def handle_rpc_result(message : Message)
      ...
      begin
        ...
        self.rpc_results.push({msg_id, Error::RequestError::Deserialize.new(ex)})
      end
      ...
      when TL::Root::RpcError::CONSTRUCTOR_ID
        self.rpc_results.push({
          msg_id,
          begin
            TL::Root::RpcError.from_bytes(result)
          rescue ex
            Error::RequestError::Deserialize.new(ex)
          end,
        })
      ...
      when GzipPacked::CONSTRUCTOR_ID
        body = begin
          gzip = GzipPacked.from_bytes(result)
          x = gzip.decompress
    ...
    def self.store_own_updates(body)
      body_id = UInt32.from_bytes(body)
      if UPDATE_IDS.any? { |id| body_id == id }
        # TODO: somehow signal that this updates is our own, to avoid getting into nasty loops
        self.updates.push(body)
      end
    rescue ex
    ...
    def handle_ack(message : Message)
      ...
      _ack = TL::Root::MsgsAck.from_bytes(message.body, boxed: true)
    end
    ...
    def handle_bad_notification(message : Message)
      bad_msg = TL::Root::IBadMsgNotification.from_bytes(message.body, boxed: true)
      bad_msg = case bad_msg
                when TL::Root::BadMsgNotification
                  bad_msg
                when TL::Root::BadServerSalt
                  self.rpc_results.push(
                    x.bad_msg_id,
                    Error::RequestError::BadMessage.new(code: x.error_code))
      ...
      self.rpc_results.push(
        bad_msg.bad_msg_id,
        Error::RequestError::BadMessage.new(code: bad_msg.error_code))

      case bad_msg.error_code
      when 16, 17
        # Sent `msg_id` was too low or too high (our `time_offset` is wrong).
    ...
    def handle_detailed_info(message : Message)
      ...
      msg_detailed = TL::Root::IMsgDetailedInfo.from_bytes(message.body, boxed: true)
      case msg_detailed
      when TL::Root::MsgDetailedInfo, TL::Root::MsgNewDetailedInfo
        self.pending_ack.push(msg_detailed.answer_msg_id)
      end
    end
    ...
    def handle_future_salts(message : Message)
      salts = TL::Root::FutureSalts.from_bytes(message.body, boxed: true)
      self.rpc_results.push(
        salts.req_msg_id,
        message.body.clone)
    end
    ...
    def handle_pong(message : Message)
      pong = TL::Root::Pong.from_bytes(message.body)
      self.rpc_results.push(
        pong.msg_id,
        message.body.clone)
    end
    ...
    def handle_new_session_created(message : Message)
      ...
      new_session = TL::Root::NewSessionCreated.from_bytes(message.body, boxed: true)
    ...
    def handle_container(message : Message)
      container = MessageContainer.from_bytes(message.body, boxed: true)
    ...
    def handle_gzip_packed(message : Message)
      container = GzipPacked.from_bytes(message.body, boxed: true)
      self.process_message(message.clone.tap { |msg|
    ...
    def push(request : Bytes) : Int64?
      # TODO: rather than taking in bytes, take requests, serialize them in place,
      # and if too large drop the last part of the buffer
      ...
      if !self.pending_ack.empty?
        body = TL::Root::MsgsAck.new(msg_ids: self.pending_ack)
        self.pending_ack.clear
        self.serialize_msg(body.to_bytes(boxed: true), false)
      end
      ...
      # connection but are so uncommon it's not worth raising.
      unless request.size + Message::SIZE_OVERHEAD < MessageContainer::MAXIMUM_SIZE
        raise "Unreachable"
      end

      # Serialized requests will always be correctly padded.
      unless request.size % 4 == 0
        raise "Unreachable"
      end
      ...
      body = request
      if threshold = self.compression_threshold
        if request.size > threshold
          compressed = GzipPacked.new(request).to_bytes(boxed: true)
      ...
      new_size = self.buffer.size + body.size + Message::SIZE_OVERHEAD
    ...
    def deserialize(payload : Bytes) : Deserialization
      ...
      # state is cleaned and returned.
      result = Deserialization.new(
        rpc_results: self.rpc_results,
        updates: self.updates,
      )

      self.rpc_results.clear
      self.updates.clear
    ...
    class Builder
      ...
      def finish(auth_key : Crypto::AuthKey)
        Encrypted.new(
          auth_key,
          time_offset: self.time_offset,
          salt: self.first_salt,
          client_id: IO::ByteFormat::LittleEndian.decode(Int64, Random::Secure.random_bytes(8)),
          sequence: 0,
          last_msg_id: 0_i64,
          pending_ack: [] of Int64,
          compression_threshold: self.compression_threshold,
          rpc_results: [] of Tuple(Int64, Bytes | Error::RequestError),
          updates: [] of Bytes,
