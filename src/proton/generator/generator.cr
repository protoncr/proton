...
module Proton::Generator
  ...
  def generate_tl_types(
    ...
    *,
    ...
    Dir["#{__DIR__}/templates/**/*.cr.j2"].each do |template|
      basename = File.basename(template, ".cr.j2")
      next if basename == "ns_file"

      dirname = Path[File.dirname(template)].relative_to(Path[__DIR__, "templates"]).normalize
